...
plt.errorbar(N_s, MEFF, yerr = MEFF_E, fmt = "x", label = r"$m_\mathrm{eff}$", color = "C0", capsize = 3)
plt.hlines(AVG, N_s.min(), N_s.max(), label = r"average $m_\mathrm{eff}$", color = "C1")
...
plt.figure(figsize=(9,6))
plt.plot(N_s, MEFF_E, ".", label = r"$\sigma_{m_\mathrm{eff}}$", color = "C0")
plt.hlines(ERR, N_s.min(), N_s.max(), label = r"average $m_\mathrm{eff}$", color = "C1")
plt.legend(loc = "best")
plt.yscale("log")
plt.grid()
plt.xlabel(r"$N$")
